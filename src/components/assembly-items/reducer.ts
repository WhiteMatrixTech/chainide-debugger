...
const reducedOpcode = (opCodes: any, payload: any) => {
  const length = 100;
  ...
  return {
    ...
    nextIndexes: opCodes.nextIndexes.map((index: any) => index - bottom),
    display: opCodes.code.slice(bottom, top),
    returnInstructionIndexes: payload.returnInstructionIndexes.map(
      (index: any) => index.instructionIndex - bottom
    ),
    outOfGasInstructionIndexes: payload.outOfGasInstructionIndexes.map(
      (index: any) => index.instructionIndex - bottom
    )
  };
};
